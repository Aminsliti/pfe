...
- All non-public backend routes require an `Authorization: Bearer <token>` header.
- The backend issues signed JWT access tokens from `POST /api/login`; `AuthContext` stores the token and injects it automatically.
- Set a strong `JWT_SECRET` before deployment. Production requires at least 32 characters.
