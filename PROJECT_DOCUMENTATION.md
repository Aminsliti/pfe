...
- persists the current user, permissions, and JWT access token in `localStorage`
- injects `Authorization: Bearer <token>` automatically into API requests
- exposes role helpers such as `hasRole`, `hasAnyRole`, and `hasPermission`
- exposes CRUD helpers for users and roles

Implementation detail:

- authentication uses signed HS256 JWT access tokens issued by the backend
- the backend resolves the request user from the token subject, then reloads roles and permissions from PostgreSQL
- public endpoints such as login and password reset bypass bearer-token validation
...
- non-public API requests must provide a valid bearer token
...
4. Backend signs a JWT access token with `JWT_SECRET` and returns token metadata.
5. Frontend stores `currentUser`, `permissions`, and the token in `localStorage`.
6. Future API requests automatically receive the `Authorization: Bearer <token>` header.
...
- `JWT_EXPIRES_IN` accepts values such as `15m`, `8h`, or `7d`; the default is `8h`.
- The backend sets basic security headers including `X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy`, and a restrictive browser permissions policy.
...
- The backend uses signed JWT bearer tokens for authenticated API calls; logout clears the client token and presence session.
