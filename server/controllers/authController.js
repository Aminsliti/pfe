...
import { createNotification } from '../services/collaboration/collaboration.js';
import { createAccessToken } from '../services/auth/jwt.js';
import { sendPlatformEmail } from '../services/mail/mailer.js';
...
function buildAuthenticatedPayload(requestUser, sessionId = null) {
  const tokenDetails = createAccessToken(requestUser, { sessionId });
...
function sendAuthenticatedPayload(res, requestUser, sessionId = null) {
  res.set('Cache-Control', 'no-store');
  res.json(buildAuthenticatedPayload(requestUser, sessionId));
}
...
router.post('/login', async (req, res) => {
  try {
    ...
    const presenceSessionId = normalizePresenceSessionId(sessionId) || randomUUID();
    await markUserPresenceSession(user.id, presenceSessionId);

    const requestUser = await buildRequestUser(user.id);

    sendAuthenticatedPayload(res, requestUser, presenceSessionId);
  } catch (error) {
...
router.get('/session', async (req, res) => {
  try {
    ...
    sendAuthenticatedPayload(res, requestUser, req.presenceSessionId || null);
  } catch (error) {
