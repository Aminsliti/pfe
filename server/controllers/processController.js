...
} from '../services/process/processDiff.js';
...
} from '../services/process/processNarrative.js';
...
async function sendProcessReport(req, res, { format = 'html', diagramImageDataUrl = null } = {}) {
  ...
  if (format === 'pdf') {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="${filenameBase} - procedure-manual.pdf"`);
  ...
  if (format === 'docx' || format === 'word') {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="${filenameBase} - procedure-manual.docx"`);
    res.send(await buildProcessReportDocx(reportProcess, explanation, { diagramImageDataUrl, workflow }));
    return;
  }

  res.setHeader('Content-Type', 'text/html; charset=utf-8');
  res.setHeader('Content-Disposition', `attachment; filename="${filenameBase} - procedure-manual.html"`);
