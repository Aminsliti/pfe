...
function buildDisplayList(values = [], fallback = 'Not specified') {
...
function hasMeaningfulValue(value = '') {
  const normalized = normalizeText(value);
  return Boolean(normalized) && normalized.toLowerCase() !== 'not specified';
}

function joinMatrixCellValues(values = [], separator = ' | ', fallback = 'Not specified') {
...
function formatActivityTypeLabel(type = '') {
  const labels = {
    ...
    callActivity: 'Called sub-process',
  };
...
function buildActivityDescription({
  ...
  if (type === 'subProcess' || type === 'callActivity') {
    details.push('Included as a sub-process in the manual.');
  }

  if (hasMeaningfulValue(inputs)) {
    details.push(`Inputs: ${normalizeText(inputs)}`);
  }
  if (hasMeaningfulValue(outputs)) {
    details.push(`Outputs: ${normalizeText(outputs)}`);
  }
  if (hasMeaningfulValue(systems)) {
    details.push(`Systems: ${normalizeText(systems)}`);
  }
  if (hasMeaningfulValue(rules)) {
    details.push(`Rules: ${normalizeText(rules)}`);
  }
...
function buildActivityRows(process = {}, documentationById = new Map(), actorAssignments = [], manualData = {}) {
  ...
  return parseNamedElementsWithAttrs(process.bpmn_xml || '', ACTIVITY_ELEMENT_NAMES).map((element, index) => {
    ...
    const systems = isSystemActivity ? element.name || element.id || 'System interaction' : 'Not specified';
    ...
    return {
      ...
      actor: actor?.actorName || fallbackActor || 'Unassigned',
      documentation,
...
function buildTaskWhenValue(activityRows = [], index = 0, manualData = {}, startEnd = {}) {
  ...
  if (index === 0) {
    values.push(normalizeText(manualData.trigger || startEnd.trigger || 'At the start of the process'));
    if (hasMeaningfulValue(manualData.frequency)) {
      values.push(`Frequency: ${manualData.frequency}`);
    }
  } else if (previousActivity) {
    values.push(`After ${previousActivity}`);
  } else {
    values.push(`Step ${index + 1} of the process`);
  }

  if (index === activityRows.length - 1 && expectedResult) {
    values.push(`Until ${expectedResult}`);
  }
...
function buildTaskWhyValue(process = {}, activityRows = [], index = 0, manualData = {}, startEnd = {}) {
  ...
  if (nextActivity) {
    ...
    values.push(`Allows obtaining ${expectedResult}`);
  } else {
    values.push('Contributes to the expected result of the process');
  }
...
function buildWhatWhoWhenWhyMatrix(process = {}, manualData = {}, accountable = [], activityRows = [], startEnd = {}) {
      ...
      rows: [
        {
          ...
          what: 'Not specified',
          who: accountable[0] || manualData.owner || 'Unassigned',
          when: normalizeText(manualData.trigger || startEnd.trigger || 'Not specified') || 'Not specified',
          why: normalizeText(manualData.objective || process.description || 'Not specified') || 'Not specified',
        },
      ],
    };
  }
  ...
  const columns = [
    { key: 'activity', label: 'Activity', width: 24 },
  ...
  const rows = activityRows.map((activity, index) => ({
    ...
    who: hasMeaningfulValue(activity.actor)
      ? activity.actor
      : (accountable[0] || manualData.owner || 'Unassigned'),
...
function dropPlaceholderRows(rows = [], keyField = 'name') {
  ...
  return rows.filter((row) => normalizeText(row?.[keyField]).toLowerCase() !== 'not specified');
}
...
function buildSupportObjectMatrix(process = {}, manualData = {}, activityRows = []) {
  ...
  const createDataRow = (entry = {}) => {
    ...
    return {
      ...
      description: normalizeText(entry.description || supportDocumentationById.get(String(entry.id || '')) || `Support data used in ${process.name || 'the process'}`) || 'Not specified',
      format: normalizeText(entry.format) || inferDataFormat(label),
      source: normalizeText(entry.source) || joinMatrixCellValues(relations.sources, ', ', 'Not specified'),
      destination: normalizeText(entry.destination) || joinMatrixCellValues(relations.destinations, ', ', 'Not specified'),
      criticality: normalizeText(entry.criticality) || inferCriticality(label),
    };
  };
  ...
  const createDocumentRow = (entry = {}) => {
    ...
    return {
      name: label || 'Not specified',
      type: normalizeText(entry.type) || inferDocumentType(label),
      generated_by: normalizeText(entry.generated_by || entry.generatedBy) || joinMatrixCellValues(relations.sources, ', ', 'Not specified'),
      output_of: normalizeText(entry.output_of || entry.outputOf) || joinMatrixCellValues(relations.destinations, ', ', 'Not specified'),
      version: normalizeText(entry.version || 'Not specified') || 'Not specified',
    };
  };

  const createSystemRow = (label = '', role = '') => ({
    name: normalizeText(label) || 'Not specified',
    ...
    sections: [
      ...
      },
      {
        ...
        columns: [
          ...
          { key: 'generated_by', label: 'Generated by', width: 20 },
          { key: 'output_of', label: 'Output of', width: 20 },
          { key: 'version', label: 'Version', width: 12 },
        ],
        rows: documentRows.length ? documentRows : [{
          name: 'Not specified',
          type: 'Not specified',
          generated_by: 'Not specified',
          output_of: 'Not specified',
      ...
      },
      {
        ...
        columns: [
          { key: 'name', label: 'Application Name', width: 34 },
          { key: 'role', label: 'Role in the process', width: 66 },
        ],
        rows: systemRows.length ? systemRows : [{
          name: 'Not specified',
          role: 'No supporting system detected.',
        }],
      },
    ],
  };
...
function buildKpiRows(manualData = {}) {
  ...
  const detailRows = manualData.kpi_details.map((entry) => ({
    name: entry.name,
    target: entry.target || 'Not specified',
  ...
  return rows.length
    ? rows
    : [
        { name: 'Processing time', target: 'Not specified', source: 'To be defined' },
        { name: 'Processed volume', target: 'Not specified', source: 'To be defined' },
        { name: 'Compliance rate', target: 'Not specified', source: 'To be defined' },
      ];
}
...
function buildRiskRows(risks = []) {
  const baseRows = risks.length
    ? risks.map((risk) => ({
        ...
        description: risk.description || 'Not specified',
  ...
  return baseRows.length
    ...
    : [{
        ...
        category: '-',
        element: '-',
        description: 'No risks are attached to the diagram.',
...
export function buildProcedureManual(process = {}, workflow = null, explanation = null) {
  ...
  const identityRows = [
    ...
    { label: 'Process Name', value: displayValue(process.name) },
    { label: 'Objective', value: displayValue(manualData.objective, process.description || 'Not specified') },
    { label: 'Owner', value: displayValue(manualData.owner, accountable[0] || process.created_by_name || 'Not specified') },
    { label: 'Scope', value: displayValue(manualData.scope, process.category_name || 'Not specified') },
    { label: 'Trigger', value: displayValue(manualData.trigger, startEnd.trigger || 'Not specified') },
    { label: 'Expected Result', value: displayValue(manualData.expected_result, startEnd.expectedResult || 'Not specified') },
    { label: 'Frequency', value: displayValue(manualData.frequency) },
    { label: 'Context', value: displayValue(manualData.context, process.description || 'Not specified') },
  ];
    ...
    ? `
      <div class="section">
        ...
        <div class="diagram-card">
          <img src="${escapeHtml(options.diagramImageDataUrl)}" alt="Process diagram" />
        </div>
      </div>
    `
    : '';
...
<head>
  <meta charset="utf-8" />
  <title>${escapeHtml(process.name || 'Process')} - Procedure Manual</title>
  <style>
    @import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&display=swap');
    body { font-family: 'Inter', -apple-system, sans-serif; margin: 0; padding: 40px 20px; color: #1e293b; background: #f1f5f9; line-height: 1.5; }
    .report-shell { max-width: 1100px; margin: 0 auto; background: #ffffff; border-radius: 12px; padding: 48px; box-shadow: 0 4px 6px -1px rgb(0 0 0 / 0.1), 0 2px 4px -2px rgb(0 0 0 / 0.1); border: 1px solid #e2e8f0; }
    
    .header-meta { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 40px; border-bottom: 2px solid #f1f5f9; padding-bottom: 24px; }
    .hero h1 { margin: 8px 0 0; font-size: 32px; font-weight: 800; color: #0f172a; letter-spacing: -0.025em; }
    .eyebrow { display: inline-block; color: #2563eb; font-size: 13px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.1em; }
    
    .section { margin-top: 48px; }
    .section h2 { margin: 0 0 16px; font-size: 22px; font-weight: 700; color: #334155; border-left: 4px solid #2563eb; padding-left: 16px; }
    .subsection { margin-top: 24px; }
    .subsection h3 { margin: 0 0 12px; font-size: 18px; font-weight: 600; color: #475569; }
    
    .section p { margin: 0 0 16px; color: #64748b; font-size: 15px; }
    .diagram-description { margin-top: 8px; color: #475569; font-size: 16px; }
    
    .diagram-card { margin-top: 24px; border: 1px solid #e2e8f0; border-radius: 8px; padding: 24px; background: #f8fafc; text-align: center; }
    .diagram-card img { max-width: 100%; height: auto; border-radius: 4px; box-shadow: 0 1px 3px 0 rgb(0 0 0 / 0.1); }
    
    .table-wrap { overflow-x: auto; margin-top: 16px; border-radius: 8px; border: 1px solid #e2e8f0; }
    table { width: 100%; border-collapse: collapse; font-size: 14px; }
    th { background: #f8fafc; color: #475569; font-weight: 600; text-transform: uppercase; font-size: 12px; letter-spacing: 0.05em; padding: 12px 16px; border-bottom: 2px solid #e2e8f0; text-align: left; }
    td { padding: 12px 16px; border-bottom: 1px solid #f1f5f9; vertical-align: top; color: #334155; }
    tr:last-child td { border-bottom: none; }
    tr:nth-child(even) { background-color: #fcfdfe; }

    .risk-pill { display: inline-block; border-radius: 6px; padding: 4px 10px; font-weight: 700; font-size: 11px; text-transform: uppercase; letter-spacing: 0.05em; }
    .risk-pill--low { background: #16a34a; color: #ffffff; }
    .risk-pill--medium { background: #eab308; color: #ffffff; }
    .risk-pill--high { background: #ea580c; color: #ffffff; }
    .risk-pill--critical { background: #dc2626; color: #ffffff; }

    .report-footer { margin-top: 64px; padding-top: 24px; border-top: 1px solid #e2e8f0; display: flex; justify-content: space-between; color: #94a3b8; font-size: 12px; font-weight: 500; }

    @media print { 
      body { padding: 0; background: #fff; } 
      .report-shell { box-shadow: none; border: none; padding: 0; max-width: none; }
      .header-meta { margin-bottom: 24px; }
    }
  </style>
</head>
<body>
  <div class="report-shell">
    <div class="header-meta">
      <div class="hero">
        <span class="eyebrow">Procedure Manual</span>
        <h1>${escapeHtml(process.name || `Process #${process.id || ''}`)}</h1>
      </div>
      <div style="text-align: right; color: #94a3b8; font-size: 12px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em;">
    ...
    ${renderTable('5.1 KPI Matrix', [
      { key: 'name', label: 'KPI' },
      { key: 'target', label: 'Target' },
      { key: 'source', label: 'Source' },
    ], manual.matrices.kpis)}

    ${renderTable('5.2 Risk Matrix', [
    ...
    <div class="report-footer">
      <div>${escapeHtml(process.name || 'Process')}</div>
      <div>
        ${process.category_name ? `Category: ${escapeHtml(process.category_name)}` : ''}
        ${process.version ? ` | Version: v${escapeHtml(process.version)}` : ''}
...
export function buildProcessReportPdf(process = {}, explanation = null, options = {}) {
  ...
  const severityFill = (value) => {
    const severity = String(value || '').toLowerCase();
    if (severity === 'low') return [0.086, 0.639, 0.290]; // strong green (#16a34a)
    if (severity === 'medium') return [0.918, 0.702, 0.031]; // strong yellow (#eab308)
    if (severity === 'high') return [0.918, 0.345, 0.047]; // strong orange (#ea580c)
    if (severity === 'critical') return [0.863, 0.149, 0.149]; // strong red (#dc2626)
    return null;
  };
  ...
  return buildPdfDocument({
    title: `Procedure Manual - ${process.name || `Process #${process.id}`}`,
    ...
    sections: [
      {
        ...
        table: {
          columns: [
            { key: 'label', label: 'Field', width: 30 },
      ...
      },
      {
        ...
        table: {
          columns: [
            { key: 'activity', label: 'Activity', width: 20 },
      ...
      },
      {
        ...
        table: {
          columns: [
            ...
            { key: 'target', label: 'Target', width: 26 },
      ...
      },
      {
        ...
        table: {
          columns: [
            ...
            { key: 'element', label: 'BPMN Element', width: 16 },
            { key: 'description', label: 'Description', width: 22 },
            { key: 'mitigation', label: 'Mitigation / Control', width: 22 },
          ],
    ...
    header: process.name || 'Procedure Manual',
    footer: metadataLine,
  });
}
...
export async function buildProcessReportDocx(process = {}, explanation = null, options = {}) {
  ...
  const severityDocxFill = (value) => {
    const severity = String(value || '').toLowerCase();
    if (severity === 'low') return '16a34a'; // strong green
    if (severity === 'medium') return 'eab308'; // strong yellow
    if (severity === 'high') return 'ea580c'; // strong orange
    if (severity === 'critical') return 'dc2626'; // strong red
    return null;
  };
  const docChildren = [
    createDocxParagraph('Procedure Manual', {
  ...
  docChildren.push(
    ...createDocxTableSection('1. Process Identity', [
      { key: 'label', label: 'Field', width: 30 },
      { key: 'value', label: 'Value', width: 70 },
    ], manual.matrices.identity)
  );
  ...
  docChildren.push(
    ...
    ...createDocxTableSection('3. Activity Matrix', [
      { key: 'activity', label: 'Activity', width: 28 },
    ...
    createDocxParagraph('4. SUPPORT OBJECT LAYER', {
      heading: HeadingLevel.HEADING_2,
    ...
    ...createDocxTableSection('5.1 KPI Matrix', [
      { key: 'name', label: 'KPI', width: 34 },
      { key: 'target', label: 'Target', width: 26 },
      { key: 'source', label: 'Source', width: 40 },
    ], manual.matrices.kpis),
    ...createDocxTableSection('5.2 Risk Matrix', [
      { key: 'title', label: 'Risk', width: 16 },
      {
        key: 'severity',
        label: 'Severity',
        width: 10,
      ...
      { key: 'mitigation', label: 'Mitigation / Control', width: 18 },
    ], manual.matrices.risks)
  );
    ...
    sections: [
      {
        ...
        headers: {
          default: new Header({
            children: [
              createDocxParagraph(process.name || 'Procedure Manual', {
        ...
        footers: {
          default: new Footer({
            children: [
              createDocxParagraph(metadataLine, {
                alignment: AlignmentType.CENTER,
                size: 16,
                color: '64748b',
              }),
