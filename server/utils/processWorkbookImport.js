...
function getCategoryDescription(name, sectionKey, parentName = '') {
  ...
  if (parentName) {
    return `Sous-categorie ${name} rattachee au domaine ${parentName} dans ${sectionLabel}.`;
  }

  return `Domaine ${name} du referentiel bancaire dans ${sectionLabel}.`;
}
...
function getProcessDescription({ name, sectionKey, categoryName, subcategoryName = '', parentProcessName = '' }) {
  ...
  if (parentProcessName) {
    return `Sous-processus ${name} rattache au processus ${parentProcessName} dans ${subcategoryName || categoryName} (${sectionLabel}).`;
  }

  if (subcategoryName) {
    return `Processus ${name} du sous-domaine ${subcategoryName}, rattache a ${categoryName} dans ${sectionLabel}.`;
  }

  return `Processus ${name} du domaine ${categoryName} dans ${sectionLabel}.`;
}
