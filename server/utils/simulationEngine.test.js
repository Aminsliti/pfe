  ...
  it('extracts tasks from BPMN XML when JSON parsing is not available', () => {
    const tasks = extractTasksFromBpmn(`
      <bpmn:definitions>
        ...
        <bpmn:adHocSubProcess id="AdHoc_Review" name="Review Cluster" />
        <bpmn:callActivity id="Call_Process" name="Reuse Process" calledElement="ClaimsProcess" />
      </bpmn:definitions>
    `);
    ...
    expect(tasks).toEqual([
      ...
      createDefaultTask('AdHoc_Review', 'Review Cluster'),
      createDefaultTask('Call_Process', 'Reuse Process'),
    ]);
