...
import { extractTasksFromDiagram } from '../utils/simulationEngine.js';
...
router.post('/process-templates', async (req, res) => {
  try {
    ...
    const { name, description, category_id, company_id, bpmn_xml, simulation_defaults = {} } = req.body;
    if (!name || !bpmn_xml) {
      return res.status(400).json({ error: 'Template name and BPMN XML are required.' });
    }

    const scopedCompanyId = isGlobalAdmin(req.user) ? (company_id || null) : (req.user.companyId || null);
    const result = await pool.query(
      `
...
router.post('/process-templates/:id/apply', async (req, res) => {
  try {
    ...
    const scopedCompanyId = isGlobalAdmin(req.user)
      ? (req.body?.company_id || template.company_id || null)
      : (req.user.companyId || template.company_id || null);

    if (!isGlobalAdmin(req.user) && template.company_id && template.company_id !== req.user.companyId) {
      return res.status(403).json({ error: 'You cannot apply a template from another company.' });
    }
