...
export function EntityCollaborationPanel({
  ...
  const load = async () => {
    ...
    try {
      ...
      setError(loadError.message || 'Unable to load collaboration.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const submitComment = async (event) => {
    ...
    try {
      ...
      const payload = await response.json();
      if (!response.ok) {
        throw new Error(payload.error || 'Unable to add the comment.');
      }
      ...
      showSnackbar(submitError.message || 'Unable to add the comment.', 'danger');
    } finally {
      setBusy('');
    }
  };
      ...
      <Card.Body>
        <div className="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-3">
          <div>
            ...
            <div className="text-muted small">Keep discussion about this item in one place.</div>
          </div>
          <Badge bg="light" text="dark">{comments.length} comment(s)</Badge>
        </div>

        {!entityId ? (
          <Alert variant="secondary" className="mb-0">
            Save this item first to add comments.
          </Alert>
        ) : (
          <>
            ...
            <div>
              <Form.Group className="mb-3">
                ...
                <Form.Control
                  ...
                  placeholder="Share a decision, note, or blocker..."
                />
              </Form.Group>
