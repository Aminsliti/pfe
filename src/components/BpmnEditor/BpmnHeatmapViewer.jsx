    ...
    const mountViewer = async () => {
      ...
      try {
        ...
        if (!cancelled) {
          ...
          setError('Unable to display the BPMN diagram for this simulation.');
        }
      }
    };
    ...
    <div className="sim-heatmap-shell">
      <div className="sim-heatmap-header">
        <div>
          ...
          <div className="text-muted small">The slowest, most delayed, or overloaded tasks are highlighted directly on the diagram.</div>
        </div>
        <div className="sim-heatmap-legend">
          <span className="sim-heatmap-chip level-low">Low</span>
          <span className="sim-heatmap-chip level-medium">Medium</span>
          <span className="sim-heatmap-chip level-high">High</span>
          <span className="sim-heatmap-chip level-very-high">Very High</span>
        </div>
      </div>
            ...
            <div key={entry.task_id} className={`sim-heatmap-rank level-${entry.level}`}>
              <div>
                ...
                <div className="text-muted small">
                  {round(entry.avg_duration)} min average ? {round(entry.avg_wait_min)} min waiting
                </div>
              </div>
