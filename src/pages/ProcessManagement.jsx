...
import { extractBpmnProcessReference, getBpmnCallActivities } from '../utils/bpmnCallActivities';
...
export function ProcessManagement({ publicView = false }) {
  ...
  const [processLinkOptions, setProcessLinkOptions] = useState([]);
  ...
  const loadProcessLinkOptions = async () => {
    if (!canViewWorkspace) {
      return;
    }
    ...
    try {
      ...
      const response = await fetch(`${API}/processes?${params.toString()}`);
      if (response.ok) {
        setProcessLinkOptions(await response.json());
      }
    } catch {}
  };
  ...
  const handleOpenLinkedProcessInEditor = async (processId) => {
    const normalizedProcessId = Number(processId);
    if (!Number.isInteger(normalizedProcessId) || normalizedProcessId <= 0) {
      throw new Error('This call activity is not linked to a process in the platform.');
    }
  ...
  const handleOpenLinkedProcessDetails = async (processId, fallbackProcess = null) => {
    const normalizedProcessId = Number(processId);
    if (!Number.isInteger(normalizedProcessId) || normalizedProcessId <= 0) {
      showMsg('This called diagram is not linked to a process record yet.', 'danger');
      return;
    }
  ...
  const previewCallActivities = useMemo(() => getBpmnCallActivities(previewBpmnXml), [previewBpmnXml]);
  ...
  const activePreviewSubprocess = previewSubprocessTrail.length
    ...
    : null;
  const processReferenceCandidates = useMemo(() => {
    const items = [];
    const seenIds = new Set();

    [selectedProcessRecord, ...processLinkOptions].forEach((process) => {
      const processId = Number(process?.id || 0);
      if (!Number.isInteger(processId) || processId <= 0 || seenIds.has(processId)) {
        return;
      }
    ...
    return items;
  }, [processLinkOptions, selectedProcessRecord]);
  const processReferenceLookup = useMemo(() => {
    const byDatabaseId = new Map();
    const byBpmnProcessId = new Map();

    processReferenceCandidates.forEach((process) => {
      const processId = Number(process?.id || 0);
      if (!Number.isInteger(processId) || processId <= 0) {
        return;
      }

      byDatabaseId.set(String(processId), process);

      const reference = extractBpmnProcessReference(
        process?.bpmn_xml || '',
        process?.name || 'Process',
        `Process_${processId}`
      );

      if (reference.processId && !byBpmnProcessId.has(reference.processId)) {
        byBpmnProcessId.set(reference.processId, process);
      }
    });
  ...
  }, [processReferenceCandidates]);
  const previewCalledDiagrams = useMemo(
    () =>
      previewCallActivities.map((callActivity) => {
        const targetProcess =
          processReferenceLookup.byDatabaseId.get(String(callActivity.linkedProcessId || '')) ||
          processReferenceLookup.byBpmnProcessId.get(String(callActivity.calledElement || '')) ||
          null;

        return {
          ...callActivity,
          targetProcess,
          targetProcessId: Number(targetProcess?.id || callActivity.linkedProcessId || 0) || null,
          resolvedName: targetProcess?.name || callActivity.linkedProcessName || callActivity.calledElement || callActivity.name,
        };
      }),
    [previewCallActivities, processReferenceLookup]
  );
    ...
    return (
      <Suspense fallback={<ModelerFallback />}>
        <BpmnEditorModeler
          ...
          importOptions={processLinkOptions
                ...
                <Col lg={12}>
                  <Card className="border-0 shadow-sm">
                    <Card.Body>
                      ...
                      {previewCalledDiagrams.length ? (
                        <div className="border rounded-4 p-3 mt-3" style={{ background: '#f8fafc' }}>
                          <div className="d-flex justify-content-between align-items-start gap-2 flex-wrap mb-3">
                            <div>
                              <h6 className="mb-1">Called diagrams</h6>
                              <div className="text-muted small">Call activities that reference another existing platform process.</div>
                            </div>
                          </div>

                          <div className="d-flex flex-column gap-2">
                            {previewCalledDiagrams.map((callActivity) => (
                              <div
                                key={callActivity.id}
                                className="border rounded-3 px-3 py-2 bg-white"
                                style={{ borderColor: '#e2e8f0' }}
                              >
                                <div className="d-flex justify-content-between align-items-start gap-3 flex-wrap">
                                  <div>
                                    <div className="fw-semibold text-dark">{callActivity.resolvedName}</div>
                                    <div className="text-muted small">
                                      Call activity: {callActivity.name}
                                    </div>
                                    <div className="text-muted small">
                                      Called element: {callActivity.calledElement || 'Not set'}
                                    </div>
                                    {!callActivity.targetProcessId ? (
                                      <div className="text-warning small mt-1">
                                        This call activity is not linked to a platform process yet.
                                      </div>
                                    ) : null}
                                  </div>
                                  <div className="d-flex gap-2 flex-wrap">
                                    {callActivity.targetProcessId ? (
                                      <Button
                                        ...
                                        onClick={() => handleOpenLinkedProcessDetails(callActivity.targetProcessId, callActivity.targetProcess)}
                                      >
                                        Open details
                                      </Button>
                                    ) : null}
                                    {callActivity.targetProcessId && !publicView && callActivity.targetProcess && canEditProcessDefinition(callActivity.targetProcess) ? (
                                      <Button
                                        type="button"
                                        size="sm"
                                        variant="primary"
                                        onClick={() => handleOpenLinkedProcessInEditor(callActivity.targetProcessId)}
                                      >
                                        Open editor
                                      </Button>
                                    ) : null}
