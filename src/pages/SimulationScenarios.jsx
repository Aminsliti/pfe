...
const statusLabel   = s => ({ draft:'Draft', running:'Running', completed:'Completed', failed:'Failed', error:'Failed' }[s] || s);
const fmt = (n, dec = 1) => n == null ? '—' : Number(n).toFixed(dec);

// --- Histogram ---------------------------------------------------------------
function Histogram({ data }) {
  if (!data?.length) return <p className="text-muted small">No data.</p>;
...
function ScenarioModal({ show, onHide, processes, onCreated }) {
  ...
  const handleSubmit = async e => {
    ...
    if (!form.process_id) { setErr('Select a process.'); return; }
  ...
  return (
    <Modal show={show} onHide={onHide} size="lg">
      <Modal.Header closeButton>
        <Modal.Title>New Simulation Scenario</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        ...
        <Form onSubmit={handleSubmit}>
          <Row>
            <Col md={8}>
              <Form.Group className="mb-3">
                ...
                <Form.Control value={form.name} onChange={e => set('name', e.target.value)} placeholder="Ex: Baseline Scenario" />
              </Form.Group>
            </Col>
            <Col md={4}>
              <Form.Group className="mb-3">
          ...
          <hr />
          <h6 className="mb-3 text-muted">Parameters</h6>
          <Row>
            ...
            <Col md={3}><Form.Group className="mb-3"><Form.Label>Warm-up (%)</Form.Label>
            ...
            <Col md={3}><Form.Group className="mb-3"><Form.Label>Cooldown to Exclude (%)</Form.Label>
            ...
            <Col md={3}><Form.Group className="mb-3"><Form.Label>Start Date</Form.Label>
              <Form.Control type="date" value={form.start_date} onChange={e => set('start_date', e.target.value)} /></Form.Group></Col>
          </Row>
          <Form.Check type="switch" id="inf-r" label="Infinite Resources"
            checked={form.infinite_resources} onChange={e => set('infinite_resources', e.target.checked)} />
          <Form.Check type="switch" id="sim-all" label="Simulate All Levels"
  ...
  return (
    <div>
      <div className="d-flex justify-content-end gap-2 mb-4">
        <Button variant="outline-secondary" size="sm" onClick={save} disabled={saving}>Update</Button>
        {msg && <span className={`align-self-center small ${msg === 'Saved' ? 'text-success' : 'text-danger'}`}>{msg}</span>}
      </div>

      <div className="sim-section">
        <div className="sim-section-title">Identification</div>
        <Row>
          <Col md={8}><Form.Group className="mb-3"><Form.Label>Name</Form.Label>
      ...
      <div className="sim-section">
        <div className="sim-section-title">Scenario Parameters</div>
        <Row className="mb-3">
          <Col md={6}>
            <Form.Label className="sim-field-label">Simulated Process</Form.Label>
            <Form.Select value={form.process_id || ''} onChange={e => set('process_id', +e.target.value)}>
        ...
        <Form.Check type="checkbox" id="csv-chk"
          label="Import CSV File with Exact Arrival Times"
        ...
        <div className="sim-info-box mb-3">
          ...
          Set the start date and number of instances; warm-up and cooldown can be excluded from results.
        </div>
        <Row>
          ...
          <Col md={3}><Form.Group className="mb-3"><Form.Label className="sim-field-label">Warm-up (%)</Form.Label>
          ...
          <Col md={3}><Form.Group className="mb-3"><Form.Label className="sim-field-label">Start Date</Form.Label>
            <Form.Control type="date" value={form.start_date?.slice(0,10) || ''} onChange={e => set('start_date', e.target.value)} /></Form.Group></Col>
          <Col md={3}><Form.Group className="mb-3"><Form.Label className="sim-field-label">Cooldown to Exclude (%)</Form.Label>
        ...
        <Form.Check type="switch" id="inf2" label="Infinite Resources"
          checked={!!form.infinite_resources} onChange={e => set('infinite_resources', e.target.checked)} />
        <Form.Check type="switch" id="all2" label="Simulate All Process Levels"
          checked={!!form.simulate_all_levels} onChange={e => set('simulate_all_levels', e.target.checked)} className="mt-1" />
      </div>
...
function ArrivalTimesCard({ scenarioId, enabled, onImported, onCleared }) {
  ...
  const load = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(payload.error || 'Unable to load imported arrivals.');
  ...
  const importCsv = async () => {
    ...
    try {
      ...
      setFile(null);
      setMsg(`${payload.count} imported arrivals.`);
      onImported?.(payload);
    } catch (error) {
      setErr(error.message);
    }
  ...
  return (
    <div className="sim-section">
      ...
      <div className="sim-info-box mb-3">
        ...
        Import a CSV containing one arrival column. Supported values: minutes, HH:MM time, or ISO date.
      </div>
      ...
      <div className="sim-results-header">
        <div className="d-flex flex-wrap align-items-center justify-content-between gap-2">
          <div className="d-flex flex-wrap align-items-center gap-2">
            ...
            {results?.simulated_at && <span>Simulation run on {new Date(results.simulated_at).toLocaleString('en-US')}</span>}
          </div>
          <div className="d-flex flex-wrap gap-3">
            {scenario.last_run_started_at && <span>Started: {new Date(scenario.last_run_started_at).toLocaleString('en-US')}</span>}
            {scenario.last_run_finished_at && <span>Finished: {new Date(scenario.last_run_finished_at).toLocaleString('en-US')}</span>}
          </div>
        </div>
      </div>
      {effectiveStatus === 'running' && (
        <div className="sim-inline-progress mb-3">
          ...
          <div>
            ...
            <div className="text-muted small">The status changes automatically to Completed or Failed when the run finishes.</div>
          </div>
        </div>
      )}
      ...
      <Row className="g-3 align-items-end">
        <Col md={8}><Form.Group><Form.Label>CSV File</Form.Label><Form.Control type="file" accept=".csv,text/csv" disabled={!enabled || uploading} onChange={e => setFile(e.target.files?.[0] || null)} /></Form.Group></Col>
        <Col md={4}><div className="d-flex gap-2"><Button disabled={!enabled || !file || uploading} onClick={importCsv}>{uploading ? 'Import...' : 'Import'}</Button><Button variant="outline-secondary" disabled={!arrivals.length} onClick={clearImport}>Clear</Button></div></Col>
      </Row>
      <div className="sim-arrivals-summary mt-3">
        <div><strong>{arrivals.length}</strong> loaded arrival(s)</div>
        <div>{loading ? 'Loading...' : arrivals.length ? `First arrival at ${fmt(arrivals[0].arrival_offset_min, 2)} min` : 'No imported arrivals'}</div>
      </div>
      {arrivals.length > 0 && <Table hover size="sm" className="sim-table mt-3 mb-0"><thead><tr><th>#</th><th>Source Value</th><th>Offset (min)</th></tr></thead><tbody>{arrivals.slice(0, 8).map(arrival => <tr key={arrival.id || arrival.arrival_order}><td>{arrival.arrival_order}</td><td>{arrival.raw_value}</td><td>{fmt(arrival.arrival_offset_min, 2)}</td></tr>)}</tbody></Table>}
    </div>
  );
}

// -- Onglet Resources ---------------------------------------------------------
function TabResources({ scenarioId }) {
  ...
  const typeLabel = t => ({ human:'Human', machine:'Machine', system:'System' }[t] || t);

  return (
    <div>
      <div className="d-flex justify-content-between align-items-center mb-3">
        <span className="text-muted small">{resources.length} resource(s)</span>
        <Button variant="outline-primary" size="sm" onClick={openNew}>Add</Button>
      </div>
      {resources.length === 0
        ? <div className="sim-empty"><i className="bi bi-people" /><p>No resources defined.</p></div>
        : (
          <Table hover size="sm" className="sim-table">
            <thead><tr><th>Name</th><th>Type</th><th>Quantity</th><th>Cost/h (€)</th><th>Availability (%)</th><th /></tr></thead>
            <tbody>
              {resources.map(r => (
                <tr key={r.id}>
                  ...
                  <td className="text-end">
                    <Button variant="link" size="sm" className="p-0 me-2" onClick={() => openEdit(r)}>Edit</Button>
                    <Button variant="link" size="sm" className="p-0 text-danger" onClick={() => del(r.id)}>Delete</Button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <Modal show={showForm} onHide={() => setShowForm(false)}>
        <Modal.Header closeButton><Modal.Title>{editing ? 'Edit' : 'Add'} Resource</Modal.Title></Modal.Header>
        <Modal.Body>
          ...
          <Form.Group className="mb-2"><Form.Label>Type</Form.Label>
            <Form.Select value={form.resource_type} onChange={e => setForm(f=>({...f, resource_type:e.target.value}))}>
              <option value="human">Human</option><option value="machine">Machine</option><option value="system">System</option>
            </Form.Select></Form.Group>
          <Row>
            <Col><Form.Group className="mb-2"><Form.Label>Quantity</Form.Label>
              <Form.Control type="number" min={1} value={form.quantity} onChange={e=>setForm(f=>({...f,quantity:+e.target.value}))} /></Form.Group></Col>
            <Col><Form.Group className="mb-2"><Form.Label>Cost/h (€)</Form.Label>
          ...
          <Form.Group><Form.Label>Availability (%)</Form.Label>
        ...
        <Modal.Footer>
          ...
          <Button variant="primary" onClick={save} disabled={saving}>{saving ? '...' : 'Save'}</Button>
        </Modal.Footer>
      </Modal>
    </div>
  );
...
function TabTaches({ scenarioId, bpmnElements }) {
  ...
  const save = async () => {
    ...
    if (res.ok) { await load(); setEditing(null); setMsg('Saved'); setTimeout(()=>setMsg(''),2000); }
    setSaving(false);
  };

  const distLabel = t => ({ fixed:'Fixedd', normal:'Normal', uniform:'Uniform', exponential:'Exponential' }[t]||t);

  return (
    <div>
      {msg && <Alert variant="success" className="py-2">{msg}</Alert>}
      {taskElements.length === 0
        ? <div className="sim-empty"><i className="bi bi-diagram-3" /><p>No tasks detected.<br/>Save your BPMN diagram first.</p></div>
        : (
          <Table hover size="sm" className="sim-table">
            <thead><tr><th>Task</th><th>Duration (min)</th><th>Distribution</th><th>Resource</th><th>Cost (€)</th><th /></tr></thead>
            <tbody>
              {taskElements.map(el => {
...
function TabFlows({ scenarioId, bpmnElements, bpmnConnections }) {
  ...
  return (
    <div>
      ...
      <div className="sim-info-box mb-3">
        <i className="bi bi-info-circle me-2" />
        Set the probability (%) for each outgoing path of exclusive gateways. The total must equal 100%.
      </div>
      {relevantFlows.length === 0
        ? <div className="sim-empty"><i className="bi bi-diagram-2" /><p>No gateways detected.<br/>Add gateways to your BPMN diagram.</p></div>
        : (
          <Table hover size="sm" className="sim-table">
            <thead><tr><th>From (gateway)</th><th>To</th><th>Label</th><th style={{width:200}}>Probability (%)</th></tr></thead>
            <tbody>
    ...
    <div>
      <div className="d-flex justify-content-end mb-3">
        <Button variant="success" onClick={run} disabled={running}>
          {running
            ? <><span className="spinner-border spinner-border-sm me-2"/>Simulation in progress…</>
            : <><i className="bi bi-play-fill me-2"/>Run Simulation</>}
        </Button>
      </div>
      {err && <Alert variant="danger">{err}</Alert>}
      {!results
        ? <div className="sim-empty"><i className="bi bi-bar-chart"/><p>No results.<br/>Click <strong>Run Simulation</strong> to start.</p></div>
        : (
          <>
            <div className="sim-results-header">
              Simulation run on {new Date(results.simulated_at).toLocaleString('en-US')}
              &nbsp;·&nbsp;{results.instances} instances &nbsp;·&nbsp;{results.active_instances} active
            </div>
            <Row className="g-3 mb-4">
              {[
                { label:'Average Duration', value:`${fmt(results.avg_duration_min)} min`, color:'primary' },
                ...
                { label:'Total Cost',    value:`${fmt(results.total_cost,2)} €`,       color:'info'    },
              ].map(k => (
            ...
            {results.histogram?.length > 0 && (
              <Card className="border-0 shadow-sm mb-4">
                <Card.Header className="bg-white"><strong>Duration Distribution (min)</strong></Card.Header>
                <Card.Body><Histogram data={results.histogram} /></Card.Body>
              </Card>
            )}
            {results.task_results?.length > 0 && (
              <Card className="border-0 shadow-sm">
                <Card.Header className="bg-white"><strong>Results by Task</strong></Card.Header>
                <Card.Body className="p-0">
                  <Table hover size="sm" className="sim-table mb-0">
                    <thead><tr><th>Task</th><th>Avg.</th><th>Min</th><th>Max</th><th>P95</th><th>Resource</th><th>Cost (€)</th></tr></thead>
                    <tbody>
...
function SimulationArrivalTimesCard({ scenarioId, enabled, onImported, onCleared }) {
  ...
  const [err, setErr] = useState('');
  const [msg, setMsg] = useState('');
  const unavailableMessage = "CSV import is not yet available on the active backend. Restart the backend server to load this feature.";
  ...
  const load = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(payload.error || 'Unable to load imported arrivals.');
      }
      setArrivals(Array.isArray(payload.arrivals) ? payload.arrivals : []);
    } catch (error) {
      setErr(error.message);
  ...
  const importCsv = async () => {
    ...
    try {
      ...
      setArrivals(Array.isArray(payload.arrivals) ? payload.arrivals : []);
      setFile(null);
      setMsg(`${payload.count} arrival(s) imported.`);
      onImported?.(payload);
    } catch (error) {
      setErr(error.message);
    } finally {
  ...
  return (
    <div className="sim-section">
      <div className="sim-section-title">Exact Arrivals (CSV)</div>
      <div className="sim-info-box mb-3">
        <i className="bi bi-upload me-2" />
        Import a CSV containing one arrival column. Supported values: minutes, HH:MM time, or ISO date.
      </div>
      {!enabled && (
        <Alert variant="secondary" className="mb-3">
          Enable the CSV import option and save the scenario before loading a file.
        </Alert>
      )}
      {err && <Alert variant="danger">{err}</Alert>}
      {msg && <Alert variant="success">{msg}</Alert>}
      ...
      {arrivals.length > 0 && (
        <Table hover size="sm" className="sim-table mt-3 mb-0">
          <thead>
            <tr><th>#</th><th>Source Value</th><th>Offset (min)</th></tr>
          </thead>
          <tbody>
    ...
    <div>
      ...
      {!results ? (
        <div className="sim-empty">
          ...
          <p>No results.<br />Click <strong>Run Simulation</strong> to start.</p>
        </div>
      ) : (
        <>
          <Row className="g-3 mb-4">
            {[
              ...
              { label:'P95', value:`${fmt(results.p95_duration_min)} min`, color:'warning' },
              { label:'P99', value:`${fmt(results.p99_duration_min)} min`, color:'secondary' },
              { label:'Total Cost', value:`${fmt(results.total_cost, 2)} EUR`, color:'info' },
              { label:'Cost / Instance', value:`${fmt(results.avg_cost_per_instance, 2)} EUR`, color:'dark' },
              { label:'Horizon', value:`${fmt(results.simulation_horizon_min)} min`, color:'success' },
              { label:'Active Instances', value:results.active_instances ?? 0, color:'primary' },
              { label:'Arrival Source', value:results.arrival_source === 'csv' ? 'CSV' : 'Generated', color:'danger' },
            ].map(kpi => (
          ...
          {results.resource_results?.length > 0 && (
            <Card className="border-0 shadow-sm mb-4">
              <Card.Header className="bg-white d-flex justify-content-between align-items-center">
                <strong>Resource Utilization Rate</strong>
                <span className="text-muted small">{results.resource_results.length} resource(s)</span>
              </Card.Header>
              <Card.Body className="p-0">
                <Table hover size="sm" className="sim-table mb-0">
                  <thead>
                ...
                <Card key={`${bottleneck.type}-${bottleneck.name}-${index}`} className={`border-0 shadow-sm sim-bottleneck-card severity-${bottleneck.severity}`}>
                  <Card.Body>
                    <div className="d-flex justify-content-between align-items-start gap-2 mb-2">
                      <div>
                        <div className="sim-kpi-label">{bottleneck.type === 'resource' ? 'Resource' : 'Task'}</div>
          ...
          <Col>
            <div className="d-flex justify-content-between align-items-center">
              <h4 className="mb-0 fw-bold">
                <i className="bi bi-clock-history me-2 text-primary"/>Simulation Scenarios
              </h4>
              <Button variant="primary" onClick={() => setShowCreate(true)}>
                <i className="bi bi-plus-lg me-1"/>New Scenario
              </Button>
            </div>
          </Col>
        </Row>
            ...
            ? (
              <Card className="border-0 shadow-sm">
                <Card.Body className="text-center py-5 text-muted">
                  ...
                  <p className="mb-0">No simulation scenarios.</p>
                  <Button variant="primary" className="mt-3" onClick={()=>setShowCreate(true)}>
                    <i className="bi bi-plus-lg me-1"/>Create the first scenario
                  </Button>
                </Card.Body>
              </Card>
            )
                        ...
                        {s.results && (
                          <div className="mt-2 pt-2 border-top d-flex gap-3" style={{fontSize:12}}>
                            <span className="text-primary fw-bold">
                              <i className="bi bi-stopwatch me-1"/>{fmt(s.results.avg_duration_min)} min avg.
                            </span>
    ...
    <Container fluid className="py-4">
        ...
        <Col>
          <div className="d-flex align-items-center gap-2">
            <button className="btn btn-link p-0 text-muted" onClick={()=>setActiveScenario(null)}>
              <i className="bi bi-arrow-left me-1"/>Scenarios
            </button>
      ...
      <Card className="border-0 shadow-sm">
        ...
        <Card.Body className="p-4">
          ...
          {activeTab==='ressources' && <TabResources scenarioId={activeScenario.id} />}
          {activeTab==='taches' && (
