...
import {
  ...
  Alert, Badge, Table, Tabs, Tab, ProgressBar,
...
const statusVariant = s => ({
  draft: 'secondary', running: 'warning', completed: 'success', error: 'danger',
}[s] || 'secondary');

const statusLabel = s => ({
  draft: 'Brouillon', running: 'En cours…', completed: 'Complété', error: 'Erreur',
}[s] || s);

const fmt = (n, dec = 1) => n == null ? '—' : Number(n).toFixed(dec);

// ─── Mini bar chart pour l'histogramme ───────────────────────────────────────
...
// ══════════════════════════════════════════════════════════════════════════════
//  Formulaire scénario (modal création)
// ══════════════════════════════════════════════════════════════════════════════
function ScenarioModal({ show, onHide, processes, onCreated }) {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      if (!res.ok) { const e2 = await res.json(); throw new Error(e2.error); }
    ...
    <Modal show={show} onHide={onHide} size="lg">
      ...
      <Modal.Body>
        ...
        <Form onSubmit={handleSubmit}>
          <Row>
            <Col md={8}>
              <Form.Group className="mb-3">
                <Form.Label>Nom du scénario *</Form.Label>
                <Form.Control value={form.name} onChange={e => set('name', e.target.value)}
                  placeholder="Ex : Scénario nominal" />
              </Form.Group>
            </Col>
            <Col md={4}>
              <Form.Group className="mb-3">
          ...
          <hr />
          <h6 className="mb-3 text-muted">Paramètres de simulation</h6>

          <Row>
            <Col md={3}>
              <Form.Group className="mb-3">
                <Form.Label>Instances *</Form.Label>
                <Form.Control type="number" min={1} value={form.process_instances}
                  onChange={e => set('process_instances', +e.target.value)} />
              </Form.Group>
            </Col>
            <Col md={3}>
              <Form.Group className="mb-3">
                <Form.Label>Montée en charge (%)</Form.Label>
                <Form.Control type="number" min={0} max={50} value={form.warmup_percent}
                  onChange={e => set('warmup_percent', +e.target.value)} />
              </Form.Group>
            </Col>
            <Col md={3}>
              <Form.Group className="mb-3">
                <Form.Label>Traîne à exclure (%)</Form.Label>
                <Form.Control type="number" min={0} max={50} value={form.cooldown_percent}
                  onChange={e => set('cooldown_percent', +e.target.value)} />
              </Form.Group>
            </Col>
            <Col md={3}>
              <Form.Group className="mb-3">
                <Form.Label>Date de début</Form.Label>
                <Form.Control type="date" value={form.start_date}
          ...
          <Row>
            <Col>
              <Form.Check type="switch" id="inf-res" label="Ressources infinies"
                checked={form.infinite_resources}
                onChange={e => set('infinite_resources', e.target.checked)} />
              <Form.Check type="switch" id="sim-all" label="Simuler tous les niveaux de processus"
                checked={form.simulate_all_levels}
                onChange={e => set('simulate_all_levels', e.target.checked)} className="mt-1" />
              <Form.Check type="switch" id="csv-imp" label="Import CSV heures d'arrivée"
                checked={form.import_csv_arrivals}
                onChange={e => set('import_csv_arrivals', e.target.checked)} className="mt-1" />
            </Col>
          </Row>
  ...
  return (
    <div>
      <div className="d-flex justify-content-end gap-2 mb-4">
        <Button variant="outline-secondary" size="sm" onClick={save} disabled={saving}>
      ...
      <div className="sim-section">
        ...
        <Form.Check type="checkbox" id="csv-chk"
          label="Import du fichier CSV des heures exactes d'arrivée des instances"
          checked={!!form.import_csv_arrivals}
          onChange={e => set('import_csv_arrivals', e.target.checked)} className="mb-3" />

        <div className="sim-info-box">
          <i className="bi bi-info-circle me-2" />
          Définissez la date de début de la simulation et le nombre d'instances souhaité ;
          la montée en charge et la traîne peuvent être exclues du calcul des résultats.
        </div>

        <Row className="mt-3">
          <Col md={3}>
            <Form.Group className="mb-3">
              <Form.Label className="sim-field-label">Instances de processus *</Form.Label>
              <Form.Control type="number" min={1} value={form.process_instances || 100}
                onChange={e => set('process_instances', +e.target.value)} />
            </Form.Group>
          </Col>
          <Col md={3}>
            <Form.Group className="mb-3">
              <Form.Label className="sim-field-label">Montée en charge (%)</Form.Label>
              <Form.Control type="number" min={0} max={50} value={form.warmup_percent || 5}
                onChange={e => set('warmup_percent', +e.target.value)} />
            </Form.Group>
          </Col>
          <Col md={3}>
            <Form.Group className="mb-3">
              <Form.Label className="sim-field-label">Date de début *</Form.Label>
              <Form.Control type="date" value={form.start_date?.slice(0,10) || ''}
                onChange={e => set('start_date', e.target.value)} />
            </Form.Group>
          </Col>
          <Col md={3}>
            <Form.Group className="mb-3">
              <Form.Label className="sim-field-label">Traîne à exclure (%)</Form.Label>
              <Form.Control type="number" min={0} max={50} value={form.cooldown_percent || 10}
                onChange={e => set('cooldown_percent', +e.target.value)} />
            </Form.Group>
          </Col>
        </Row>

        <div className="sim-info-box mt-1">
          <i className="bi bi-info-circle me-2" />
          <span><strong>Ressources infinies</strong> : simule sans tenir compte des ressources.<br />
          <strong>Simuler tous les niveaux</strong> : décompose tous les sous-processus.</span>
        </div>
  ...
  return (
    <div>
      <div className="d-flex justify-content-between align-items-center mb-3">
        <span className="text-muted small">{resources.length} ressource(s) définie(s)</span>
        <Button variant="outline-primary" size="sm" onClick={openNew}>+ Ajouter une ressource</Button>
      </div>
      ...
      {resources.length === 0
        ...
        : (
          <Table hover size="sm" className="sim-table">
            <thead><tr><th>Nom</th><th>Type</th><th>Quantité</th><th>Coût/h (€)</th><th>Disponibilité (%)</th><th /></tr></thead>
            <tbody>
              {resources.map(r => (
                <tr key={r.id}>
      ...
      <Modal show={showForm} onHide={() => setShowForm(false)}>
        ...
        <Modal.Body>
          ...
          <Row>
            ...
            <Col><Form.Group className="mb-2"><Form.Label>Coût/heure (€)</Form.Label>
          ...
          <Form.Group className="mb-2"><Form.Label>Disponibilité (%)</Form.Label>
...
// ══════════════════════════════════════════════════════════════════════════════
//  Onglet Données de simulation des tâches
// ══════════════════════════════════════════════════════════════════════════════
function TabTaches({ scenarioId, bpmnElements }) {
  ...
  // Fusionner les éléments BPMN (tâches) avec les données de simulation
  const taskElements = bpmnElements.filter(e =>
    e.type?.endsWith('Task') || e.type === 'subProcess'
  );

  const getTaskData = id => tasks.find(t => t.task_id === id);

  const openEdit = el => {
    const existing = getTaskData(el.id);
    setForm({
      task_id:       el.id,
      task_name:     el.label || el.id,
      duration_min:  existing?.duration_min  ?? 30,
      duration_type: existing?.duration_type ?? 'fixed',
      duration_std:  existing?.duration_std  ?? 0,
  ...
  return (
    <div>
      ...
      {taskElements.length === 0
        ? <div className="sim-empty"><i className="bi bi-diagram-3" /><p>Aucune tâche détectée dans le diagramme BPMN.<br/>Enregistrez d'abord votre diagramme.</p></div>
        : (
          <Table hover size="sm" className="sim-table">
            <thead>
              <tr><th>Tâche</th><th>Durée (min)</th><th>Distribution</th><th>Ressource</th><th>Coût (€)</th><th /></tr>
            </thead>
            <tbody>
              {taskElements.map(el => {
                ...
                return (
                  <tr key={el.id} className={isEdit ? 'table-primary' : ''}>
                    <td><strong>{el.label || el.id}</strong><br/><span className="text-muted" style={{fontSize:10}}>{el.type}</span></td>
                    {isEdit ? (
                      <>
                        <td>
...
// ══════════════════════════════════════════════════════════════════════════════
//  Onglet Probabilités des enchainements
// ══════════════════════════════════════════════════════════════════════════════
function TabFlows({ scenarioId, bpmnElements, bpmnConnections }) {
  ...
  // Filtrer les connexions qui sortent d'un gateway
  ...
  return (
    <div>
      ...
      <div className="sim-info-box mb-3">
        <i className="bi bi-info-circle me-2" />
        Définissez la probabilité (%) de chaque sortie des passerelles exclusives.
        La somme doit idéalement être égale à 100%.
      </div>
      ...
      {relevantFlows.length === 0
        ...
        : (
          <Table hover size="sm" className="sim-table">
            <thead>
              <tr><th>De (gateway)</th><th>Vers</th><th>Libellé</th><th style={{width:180}}>Probabilité (%)</th></tr>
            </thead>
            <tbody>
              {relevantFlows.map(conn => {
                const existing = getFlowProb(conn.id);
                const [val, setVal] = useState(existing?.probability ?? 50);
                return (
                  <tr key={conn.id}>
                    ...
                    <td>
                      <div className="d-flex align-items-center gap-2">
                        <Form.Control type="number" size="sm" min={0} max={100}
                          value={val} style={{width:70}}
                          onChange={e => setVal(+e.target.value)} />
                        <ProgressBar now={val} style={{flex:1, height:6}} />
                        <Button size="sm" variant="outline-primary" disabled={saving[conn.id]}
                          onClick={() => save(conn, val)}>
                          {saving[conn.id] ? '…' : '✓'}
                        </Button>
                      </div>
                    </td>
      ...
      {!results
        ? (
          <div className="sim-empty">
            ...
            <p>Aucun résultat disponible.<br />Cliquez sur <strong>Simuler</strong> pour lancer la simulation.</p>
          </div>
        )
        : (
          <>
            <div className="sim-results-header">
              ...
              &nbsp;·&nbsp;{results.active_instances} actives
            </div>
            ...
            <Row className="g-3 mb-4">
              {[
                ...
                { label:'Durée min',      value:`${fmt(results.min_duration_min)} min`, color:'success' },
                { label:'Durée max',      value:`${fmt(results.max_duration_min)} min`, color:'danger'  },
              ...
              <Card className="border-0 shadow-sm">
                ...
                <Card.Body className="p-0">
                  <Table hover size="sm" className="sim-table mb-0">
                    <thead>
                      <tr><th>Tâche</th><th>Moy. (min)</th><th>Min</th><th>Max</th><th>P95</th><th>Ressource</th><th>Coût (€)</th></tr>
                    </thead>
                    <tbody>
                      {results.task_results.map(t => (
                        <tr key={t.task_id}>
                          <td><strong>{t.task_name || t.task_id}</strong></td>
                          <td>{fmt(t.avg_duration)}</td>
...
// ══════════════════════════════════════════════════════════════════════════════
//  Page principale — SimulationScenarios
// ══════════════════════════════════════════════════════════════════════════════
export default function SimulationScenarios() {
  ...
  const [activeScenario, setActiveScenario] = useState(null); // scénario ouvert
  ...
  // Extraire les éléments BPMN du processus lié
  const bpmnData = (() => {
    try {
      if (!activeScenario?.bpmn_xml) return { elements:[], connections:[] };
      const d = JSON.parse(activeScenario.bpmn_xml);
      return { elements: d.elements||[], connections: d.connections||[] };
    } catch { return { elements:[], connections:[] }; }
  })();
  // Chercher aussi le bpmn_xml depuis la liste des processus si nécessaire
  const linkedProcess = processes.find(p => p.id === activeScenario?.process_id);
  const bpmnParsed = (() => {
    if (bpmnData.elements.length) return bpmnData;
    try {
      if (!linkedProcess?.bpmn_xml) return { elements:[], connections:[] };
      const d = JSON.parse(linkedProcess.bpmn_xml);
      return { elements: d.elements||[], connections: d.connections||[] };
    } catch { return { elements:[], connections:[] }; }
  })();
  ...
  if (!activeScenario) {
    return (
      <Container fluid className="py-4">
                        ...
                        {s.results && (
                          <div className="mt-2 pt-2 border-top d-flex gap-3" style={{fontSize:12}}>
                            <span className="text-primary fw-bold">
                              ...
                              {fmt(s.results.avg_duration_min)} min moy.
                            </span>
                            <span className="text-success">
                              <i className="bi bi-currency-euro me-1"/>
                              {fmt(s.results.total_cost,2)} €
                            </span>
                          </div>
                        )}
                      </Card.Body>
        ...
        <ScenarioModal
          show={showCreate} onHide={() => setShowCreate(false)}
          processes={processes}
      ...
      <Card className="border-0 shadow-sm">
        ...
        <Card.Body className="p-4">
          ...
          {activeTab === 'ressources' && (
            <TabRessources scenarioId={activeScenario.id} />
          )}
          {activeTab === 'taches' && (
            <TabTaches
