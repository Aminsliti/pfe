...
const STATUS_META = {
  ...
  review: { label: 'In Review', bg: '#dbeafe', color: '#1d4ed8' },
  ...
  return (
    ...
    >
      <div className="card-body d-flex flex-column gap-2 p-3">
        <div className="small text-uppercase fw-bold opacity-75"><i className="bi bi-diagram-3 me-2" />Category</div>
        <div className="fs-6 fw-bold lh-sm">{category.name}</div>
        <div className="mt-auto d-flex flex-wrap gap-2">
          <span className="badge text-bg-light">{category.children.length} subcategory{category.children.length === 1 ? '' : 'ies'}</span>
...
function ProcessCard({ process, onOpen, publicView = false, compact = false }) {
      ...
      >
        <div className="card-body p-2 d-flex flex-column gap-2 h-100">
          <div className="small text-uppercase fw-bold opacity-75">
            ...
            {process.childCount > 0 ? 'Macro-process' : 'Process'}
          </div>
          <div
            className="fw-semibold lh-sm"
  ...
  return (
      ...
      <div className="card-body d-flex flex-column gap-2 p-3 h-100">
        <div className="small text-uppercase fw-bold opacity-75"><i className="bi bi-bezier2 me-2" />{process.childCount > 0 ? 'Macro-process' : 'Process'}</div>
        ...
        {!publicView ? (
          <div className="small opacity-75" style={{ display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
            {process.description || 'Open this process to continue navigating to the BPMN diagram.'}
          </div>
        ) : null}
        <div className="mt-auto d-flex flex-wrap gap-2">
          <span className="badge" style={{ background: status.bg, color: status.color }}>{status.label}</span>
          <span className="badge text-bg-light">v{process.version || 1}</span>
          <span className="badge text-bg-light">{process.childCount > 0 ? `${process.childCount} sub-process${process.childCount === 1 ? '' : 'es'}` : 'Final diagram'}</span>
        </div>
      </div>
    </button>
  );
  ...
  return (
    <section className="card border-0 shadow-sm" style={{ background: section.background, borderRadius: publicView ? 22 : 28 }}>
      <div className={`card-body d-flex flex-column ${publicView ? 'p-3 gap-3' : 'p-4 gap-4'}`}>
        <div className="d-flex justify-content-between align-items-start gap-3 flex-wrap" style={{ minHeight: publicView ? 82 : 142 }}>
          <div>
            <h2 className="mb-0" style={{ color: section.accent, fontSize: publicView ? '1.15rem' : undefined, lineHeight: 1.18 }}>{section.title}</h2>
          </div>
          <div className="d-flex gap-2 flex-wrap align-items-center">
            <span className="badge rounded-pill text-bg-light">{categories.length} categories</span>
            {looseProcesses.length ? <span className="badge rounded-pill text-bg-light">{looseProcesses.length} uncategorized</span> : null}
            {onToggle ? (
        ...
        {!collapsed && looseProcesses.length ? (
          <div className={`d-flex flex-column ${publicView ? 'gap-2' : 'gap-3'}`}>
            <h3 className="h5 mb-0">Uncategorized Processes</h3>
            <VerticalLane compact={publicView}>
    ...
    <section className="card border-0 shadow-sm bg-white" style={{ borderRadius: 28 }}>
      <div className="card-body p-4 d-flex flex-column gap-4">
        <div className="d-flex justify-content-between align-items-start gap-3">
          <div>
            <div className="small text-uppercase fw-bold text-danger mb-2"><i className="bi bi-diagram-2 me-2" />Category</div>
            <h2 className="mb-2">{category.name}</h2>
            <p className="mb-0 text-muted">{category.description || 'Move level by level through this branch.'}</p>
          </div>
        ...
        <div className="d-flex gap-2 flex-wrap">
          <span className="badge rounded-pill text-bg-light">{childCategories.length} subcategory{childCategories.length === 1 ? '' : 'ies'}</span>
          <span className="badge rounded-pill text-bg-light">{visibleProcesses.length} process{visibleProcesses.length === 1 ? '' : 'es'}</span>
        </div>

        {showsCategoriesOnly ? (
          <div className="d-flex flex-column gap-3">
            <h3 className="h5 mb-0">Subcategories</h3>
          ...
          <div className="d-flex flex-column gap-3">
            <h3 className="h5 mb-0">Processes</h3>
            {visibleProcesses.length ? (
              ...
              <EmptyBox title="No visible processes" text="No approved processes are visible in this category." />
            )}
          </div>
        )}
      </div>
  ...
  return (
    <section className="card border-0 shadow-sm bg-white" style={{ borderRadius: 28 }}>
      <div className="card-body p-4 d-flex flex-column gap-4">
        <div className="d-flex justify-content-between align-items-start gap-3">
          <div>
            <div className="small text-uppercase fw-bold text-danger mb-2"><i className="bi bi-bezier2 me-2" />Macro-process</div>
            <h2 className="mb-2">{process.name}</h2>
            <p className="mb-0 text-muted">{publicView ? 'Open a sub-process to continue to the diagram detail page.' : (process.description || 'Open a sub-process to continue to the detailed diagram.')}</p>
          </div>
          <button type="button" className="btn btn-outline-secondary btn-sm rounded-pill flex-shrink-0" onClick={onBack}>
            <i className="bi bi-arrow-left me-2" />Back
          </button>
        ...
        <div className="d-flex gap-2 flex-wrap">
          <span className="badge rounded-pill text-bg-light">{children.length} visible sub-process{children.length === 1 ? '' : 'es'}</span>
          <span className="badge rounded-pill text-bg-light">{process.descendantCount} level{process.descendantCount === 1 ? '' : 's'} deep</span>
        </div>
        ...
        {children.length ? (
          ...
          <EmptyBox title="No visible sub-processes" text="Adjust your filters or go back to choose another branch." />
        )}
      </div>
    </section>
  );
...
function ProcessLeafView({ process, publicView = false, navigationParam = '' }) {
  ...
  redirectParams.set('processId', String(processId));
  if (navigationParam) {
    redirectParams.set('libraryNav', navigationParam);
  }
  if (publicView) {
    redirectParams.set('view', 'tree');
    redirectParams.set('returnView', 'library');
    return <Navigate to={`/portal?${redirectParams.toString()}`} replace />;
  }

  redirectParams.set('returnTo', 'process-library');
  return <Navigate to={`/processes?${redirectParams.toString()}`} replace />;
  ...
  return (
    <section className="card border-0 shadow-sm bg-white" style={{ borderRadius: 28 }}>
      <div className="card-body p-4 d-flex flex-column gap-4">
        <div className="d-flex justify-content-between align-items-start gap-3">
          <div>
            <div className="small text-uppercase fw-bold text-danger mb-2"><i className="bi bi-image me-2" />Final BPMN Diagram</div>
            <h2 className="mb-2">{process.name}</h2>
            <p className="mb-0 text-muted">{process.description || 'You have reached the final navigation level: the detailed process diagram.'}</p>
          </div>
          <button type="button" className="btn btn-outline-secondary btn-sm rounded-pill flex-shrink-0" onClick={onBack}>
            <i className="bi bi-arrow-left me-2" />Back
          </button>
        ...
        <div className="row g-4">
          <div className="col-xl-8">
            ...
            >
              <div className="small text-muted mb-2">Click the diagram to open it.</div>
              <Suspense fallback={<div className="text-center py-5 text-muted">Rendering BPMN diagram...</div>}>
            ...
            <div className="d-flex flex-column gap-3 h-100">
              <div className="card border-0 shadow-sm" style={{ background: '#fffdfa', borderRadius: 24 }}>
                <div className="card-body d-flex flex-column gap-3">
                  <h3 className="h5 mb-0">Information</h3>
                  <div className="d-flex justify-content-between gap-3 border-bottom pb-2"><span className="text-muted">Category</span><strong className="text-end">{process.category_name || 'Uncategorized'}</strong></div>
                  <div className="d-flex justify-content-between gap-3 border-bottom pb-2"><span className="text-muted">Owner</span><strong className="text-end">{process.created_by_name || 'BPM Team'}</strong></div>
                  <div className="d-flex justify-content-between gap-3 border-bottom pb-2"><span className="text-muted">Created</span><strong className="text-end">{formatDate(process.created_at)}</strong></div>
                  <div className="d-flex justify-content-between gap-3"><span className="text-muted">Updated</span><strong className="text-end">{formatDate(process.updated_at)}</strong></div>
                </div>
              </div>
              ...
              {publicView ? (
                <div className="card border-0 shadow-sm" style={{ background: '#fffdfa', borderRadius: 24 }}>
                  <div className="card-body d-flex flex-column gap-3">
                    <div>
                      <h3 className="h5 mb-1">Procedure Manual</h3>
                    ...
                    <div className="d-flex flex-wrap gap-2">
                      ...
                      >
                        {manualPreviewLoading ? 'Loading...' : 'View Manual'}
                      </button>
                      <button
                        type="button"
              ...
              {subprocesses.length ? (
                <div className="card border-0 shadow-sm" style={{ background: '#fffdfa', borderRadius: 24 }}>
                  <div className="card-body d-flex flex-column gap-3">
                    <div>
                      <h3 className="h5 mb-1">Embedded Sub-processes</h3>
                      <p className="text-muted small mb-0">Open the same internal BPMN level that the drilldown arrow opens inside the editor.</p>
                    </div>
                      ...
                      {subprocesses.map((subprocess) => (
                        ...
                        >
                          ...
                          <div className="small text-muted mt-2">
                            {subprocess.childCount > 0 ? `${subprocess.childCount} nested view(s)` : 'Final sub-process view'}
                          </div>
                        </button>
                      ))}
                    </div>
            ...
            <div className="card border-0 bg-light-subtle">
              <div className="card-body">
                ...
                <div className="mb-3">
                  ...
                  <div className="fw-semibold">{process.category_name || 'Uncategorized'}</div>
                </div>
      ...
      <Modal show={showManualModal} onHide={() => setShowManualModal(false)} size="xl" centered>
        <Modal.Header closeButton>
          <Modal.Title>Procedure Manual</Modal.Title>
        </Modal.Header>
        <Modal.Body>
  ...
  const loadLibrary = useCallback(async () => {
    ...
    try {
      ...
      if (!processResponse.ok || !categoryResponse.ok) {
        throw new Error('Unable to load the process library.');
      }

      const processList = await processResponse.json();
      const categoryList = await categoryResponse.json();
      const normalizedCategories = Array.isArray(categoryList) ? categoryList.map(translateEntityToEnglish) : [];
      ...
      setProcesses((Array.isArray(processList) ? processList : []).map((process) => ({
        ...translateEntityToEnglish(process),
        section: normalizeSection(categorySectionById.get(Number(process.category_id))),
      })));
      setCategories(normalizedCategories);
    } catch (loadError) {
      console.error(loadError);
      setError('The process library is currently unavailable.');
    } finally {
      setLoading(false);
    }
      ...
      <div className="mx-auto d-flex flex-column gap-3" style={{ maxWidth: 1520 }}>
        ...
        {loading ? (
          ...
          currentProcess.childCount > 0 ? (
            ...
            <ProcessLeafView process={currentProcess} publicView={publicView} navigationParam={serializeNavigationParam(navigation)} />
          )
        ) : !hasRootContent ? (
          <div className="card border-0 shadow-sm text-center text-muted" style={{ borderRadius: 28 }}>
            <div className="card-body py-5">
              <i className="bi bi-search fs-1 d-block mb-3 text-danger" />
              No categories or processes match the current filters.
            </div>
          </div>
        ) : (
