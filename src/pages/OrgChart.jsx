...
async function parseApiPayload(response, fallbackMessage) {
  ...
  if (!response.ok) {
    if (response.status === 404 && response.url.includes('/api/orgchart/')) {
      throw new Error('Org chart API not found on port 3001. Restart the backend so it loads the new org chart routes.');
    }
          ...
          <Form.Group className="org-form-grid__full">
            ...
            <div className="d-flex gap-3 flex-wrap">
              <Form.Check
                ...
                label="Visible on main org chart"
              ...
              <Form.Check
                ...
                name="org-placement-mode"
                label="Internal (visible after parent Expand)"
...
export function OrgChart({ publicView = false }) {
  ...
  const loadOrgChart = async (silent = false) => {
    ...
    try {
      ...
      const nextNodes = await parseApiPayload(nodesResponse, 'Failed to load org chart nodes.');
      setNodes(nextNodes);
      ...
      setError(requestError.message || 'The org chart could not be loaded.');
    } finally {
  ...
  const navigateToFocusedNode = useCallback((nodeId, { rememberCurrent = true, closeModal = false } = {}) => {
    ...
    let normalizedNextId = null;
    if (nodeId !== null && nodeId !== undefined && nodeId !== '') {
      const parsedNextId = Number(nodeId);
      if (!Number.isInteger(parsedNextId) || !nodeIndexes.byId.has(parsedNextId)) {
        return;
      }
      normalizedNextId = parsedNextId;
    }
    ...
    if (rememberCurrent) {
      setFocusHistory((current) => [...current, normalizedCurrentId]);
    }
    setFocusedNodeId(normalizedNextId);
    if (normalizedNextId !== null) {
      setSelectedId(normalizedNextId);
    }
    if (closeModal) {
      setShowDetailsModal(false);
    }
  ...
  const jumpToFocusedTrailIndex = useCallback((index) => {
    ...
    navigateToFocusedNode(nextNode?.id || null, { rememberCurrent: true });
  }, [focusTrail, navigateToFocusedNode]);
  ...
  const goBackFromFocusedNode = useCallback(() => {
    ...
    setFocusedNodeId(previousFocus === null ? null : Number(previousFocus));
    if (previousFocus !== null && previousFocus !== undefined) {
      setSelectedId(Number(previousFocus));
    }
  }, [focusHistory]);
  ...
  const openCreateModal = (parentNode = null) => {
    ...
    if (!parentNode) {
      ...
      if (hasCompany) {
        showMessage('Only one top-level Company is allowed in the org chart.', 'danger');
        return;
      }
    ...
    if (parentNode.nodeType === 'function') {
      showMessage('A Function actor cannot have children.', 'danger');
      return;
    }
  ...
  const saveSelectedNode = async (event) => {
    ...
    if (inspectorForm.nodeType === 'company') {
      ...
      if (otherCompany) {
        showMessage('Only one Company actor is allowed.', 'danger');
        return;
      }
    }
    ...
    if (inspectorForm.nodeType === 'institute') {
      if (!parentNode || parentNode.nodeType !== 'company') {
        showMessage('An Institute actor must be directly under a Company.', 'danger');
        return;
      }
    }
    ...
    try {
      ...
      showMessage('Org chart actor updated successfully.');
      await loadOrgChart(true);
      setSelectedId(data.id);
    } catch (requestError) {
      console.error(requestError);
      showMessage(requestError.message || 'Failed to update actor.', 'danger');
    } finally {
      setSaving(false);
    }
  };
  ...
  const createNode = async (event) => {
    ...
    if (!parentNode) {
      ...
      if (createForm.nodeType !== 'company') {
        showMessage('Top-level actor must be of type Company.', 'danger');
        return;
      }
    }
    ...
    if (createForm.nodeType === 'company' && parentNode) {
      showMessage('Company actors can only be created at the top level.', 'danger');
      return;
    }

    // Institute can only be under Company.
    if (createForm.nodeType === 'institute' && (!parentNode || parentNode.nodeType !== 'company')) {
      showMessage('An Institute actor must be created directly under a Company.', 'danger');
      return;
    }
    ...
    try {
      ...
      showMessage(
        data?.placementMode === 'nested'
          ? 'New organigram actor created (inside/Expand).'
          : 'New organigram actor created (outside/main).'
      );
      ...
      console.error(requestError);
      showMessage(requestError.message || 'Failed to create actor.', 'danger');
    } finally {
      setSaving(false);
    }
  };
  ...
  const deleteSelectedNode = async () => {
    ...
    const confirmed = await confirmAction({
      title: 'Delete actor',
      message: `Delete "${selectedNode.name}"? Children will move to the deleted actor's parent.`,
      confirmLabel: 'Delete',
    ...
    try {
      ...
      showMessage('Organigram actor deleted.');
      setShowDetailsModal(false);
      setSelectedId(selectedNode.parentId || null);
      await loadOrgChart(true);
    } catch (requestError) {
      console.error(requestError);
      showMessage(requestError.message || 'Failed to delete actor.', 'danger');
    } finally {
      setSaving(false);
    }
  };
  ...
  const saveNodePosition = async (nodeId, posX, posY) => {
    ...
    try {
      ...
      console.error(requestError);
      showMessage(requestError.message || 'Failed to move actor.', 'danger');
      await loadOrgChart(true);
    } finally {
      setSaving(false);
    }
    ...
    <Container fluid className="org-page">
      ...
      <div className="org-workspace">
        <Card className={`org-board${boardFullscreen ? ' is-fullscreen' : ''}`} ref={boardRef}>
          <Card.Body>
            <div className="org-board__toolbar">
              <div className="org-board__filters">
                <InputGroup>
                  ...
                  <Form.Control value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} placeholder="Search actors, roles, or people" />
                </InputGroup>
              </div>
            ...
            {loading ? (
              <div className="org-state"><Spinner animation="border" variant="danger" /><p>Loading the org chart editor...</p></div>
            ) : visibleNodes.length === 0 ? (
              <div className="org-state">
                <div className="org-state__icon"><i className="bi bi-diagram-3"></i></div>
                <h3>No actors to display</h3>
                <p>{searchTerm ? 'Adjust the search to bring matching actors back into view.' : 'Create the first actor to start building the organization.'}</p>
                {canEdit && <Button variant="danger" onClick={() => openCreateModal()}>Create first actor</Button>}
              </div>
            ) : (
              <div
      ...
      >
        <Modal.Header closeButton className="org-modal__header">
          <Modal.Title>{selectedNode ? selectedNode.name : 'Actor details'}</Modal.Title>
        </Modal.Header>
          ...
          {selectedNode ? (
            <div className="d-flex flex-column gap-4">
              <div className="d-flex justify-content-between align-items-start gap-3 flex-wrap">
                <div>
                  <span className="org-inspector__eyebrow">Actor</span>
              ...
              {canEdit ? (
                <Form onSubmit={saveSelectedNode}>
                  ...
                  <Button variant="outline-danger" className="org-inspector__delete" onClick={deleteSelectedNode} disabled={saving}>
                    <i className="bi bi-trash me-2"></i>Delete actor
                  </Button>
                </Form>
              ) : (
                <div className="org-inspector__readonly">
                  <p>{selectedNode.description || 'No description for this actor yet.'}</p>
                </div>
              )}
              ...
              {!publicView ? (
                <div>
                  <EntityCollaborationPanel
                    ...
                    title="Actor comments and files"
                  />
                </div>
              ) : null}
            </div>
      ...
      >
        <Modal.Header closeButton className="org-modal__header">
          <Modal.Title>Create Organigram Actor</Modal.Title>
        </Modal.Header>
        <Modal.Body className="org-modal__body">
          <Form onSubmit={createNode}>
            ...
            <div className="org-modal__actions">
              ...
              <Button type="submit" variant="danger" disabled={saving}>{saving ? 'Creating...' : 'Create Actor'}</Button>
            </div>
          </Form>
        </Modal.Body>
      </Modal>
