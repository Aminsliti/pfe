...
function persistAccessToken(payload = {}) {
  if (!payload?.token) {
    return;
  }
  ...
  try {
    ...
    if (payload.expiresAt) {
      localStorage.setItem(ACCESS_TOKEN_EXPIRES_AT_KEY, payload.expiresAt);
    } else {
      localStorage.removeItem(ACCESS_TOKEN_EXPIRES_AT_KEY);
    }

    if (payload.sessionId) {
      sessionStorage.setItem(SESSION_ID_KEY, payload.sessionId);
    }
  } catch {}
}

function persistAuthenticatedSession(user, permissions, payload = {}) {
...
export function createAuthedFetch(fetchImpl = ORIGINAL_FETCH) {
  return async (input, init = undefined) => {
    ...
    if (!accessToken || !shouldAttachUserHeader(requestUrl)) {
    ...
    if (response.status === 401 || response.status === 403) {
      try {
        ...
        if (
          ...
          errorText.includes('log in again') ||
          errorText.includes('authentication required') ||
          errorText.includes('authentication token') ||
          errorText.includes('expired session')
        ) {
          clearStoredSession();
...
export function AuthProvider({ children }) {
  ...
  const pingPresence = async (fetchImpl = fetch) => {
    ...
    if (!currentSessionUser?.id || !getStoredAccessToken()) {
  ...
  const refreshCurrentUser = async () => {
    ...
    try {
      ...
      persistAuthenticatedSession(normalizedUser, nextPermissions, data);
  ...
  useEffect(() => {
    ...
    if (savedUser && getStoredAccessToken()) {
      setUser(savedUser);
