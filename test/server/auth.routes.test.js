...
describe('auth routes', () => {
  ...
  it('logs in successfully and returns permissions', async () => {
    ...
    pool.query
      ...
      .mockResolvedValueOnce(makeResult([{ name: 'manage_processes' }, { name: 'view_dashboard' }]));
    ...
    expect(response.body.permissions).toEqual(['manage_processes', 'view_dashboard']);
  });
  ...
  it('enforces global-admin-only role management and supports role CRUD endpoints', async () => {
    ...
    pool.query.mockResolvedValueOnce(makeResult([{ id: 8, name: 'Auditor', description: 'Read-only auditor' }]));
    const created = await request(adminApp).post('/api/roles').send({ name: 'Auditor', description: 'Read-only auditor' });
    expect(created.status).toBe(201);

    pool.query.mockResolvedValueOnce(makeResult([{ id: 8, name: 'Auditor', description: 'Updated' }]));
