...
describe('process routes', () => {
  ...
  it('returns a process explanation and exports the reduced procedure manual in every format', async () => {
    ...
    expect(manualJson.body.manual.matrices.activities[1].description).toContain('Included as a sub-process in the manual.');
    ...
    expect(manualJson.body.manual.matrices.whatWhoWhenWhy.rows[1].when).toContain('After Receive claim');
    ...
    expect(manualJson.body.manual.matrices.supportObjects.sections[0].title).toBe('4.1 Data');
    ...
    expect(htmlReport.text).toContain('2. What / Who / When / Why Matrix');
    expect(htmlReport.text.indexOf('2. What / Who / When / Why Matrix')).toBeLessThan(htmlReport.text.indexOf('3. Activity Matrix'));
    expect((htmlReport.text.match(/2\. What \/ Who \/ When \/ Why Matrix/g) || [])).toHaveLength(1);
    expect(htmlReport.text).not.toContain('Card Dispute Handling is represented by an approved BPMN diagram.');
    expect(htmlReport.text.indexOf('3. Activity Matrix')).toBeLessThan(htmlReport.text.indexOf('Workflow notes'));
    ...
    expect(htmlReport.text).toContain('4. SUPPORT OBJECT LAYER');
    expect(htmlReport.text).toContain('4.1 Data');
