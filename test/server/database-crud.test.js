...
describe('database CRUD contracts', () => {
  ...
  it('issues insert/select/update/delete queries for user and process CRUD flows', async () => {
    ...
    pool.query
      ...
      .mockResolvedValueOnce(makeResult([{ id: 40, name: 'CRUD Process', category_id: null, company_id: 2, created_by: 1, status: 'draft', version: 1, created_at: '2026-03-01T00:00:00.000Z', updated_at: '2026-03-01T00:00:00.000Z' }]))
      .mockResolvedValueOnce(makeResult([]))
      .mockResolvedValueOnce(makeResult([{ id: 40, name: 'CRUD Process', description: '', bpmn_xml: '<bpmn:definitions />', category_id: null, company_id: 2, status: 'draft', version: 1 }]))
      .mockResolvedValueOnce(makeResult([{ id: 40, name: 'CRUD Process Updated', category_id: null, company_id: 2, status: 'active', version: 2, updated_at: '2026-03-02T00:00:00.000Z' }]))
      .mockResolvedValueOnce(makeResult([]))
      .mockResolvedValueOnce(makeResult([{ id: 40, name: 'CRUD Process Updated', company_id: 2, bpmn_xml: '<bpmn:definitions />' }]))
